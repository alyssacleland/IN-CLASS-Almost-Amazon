...
const domEvents = (user) => {
  document.querySelector('#main-container').addEventListener('click', (e) => {
    ...
    if (e.target.id.includes('edit-book-btn')) {
      const [, firebaseKey] = e.target.id.split('--');
      getSingleBook(firebaseKey).then((bookObj) => addBookForm(user, bookObj)); // need to pass both user and book obj in b/c addBookForm takes 2 args. without passign user, the update form did not auto populate with existing book info.
